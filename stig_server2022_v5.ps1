...
# Windows Server 2022 LAN Manager authentication level must be 
configured to send NTLMv2 response only and to refuse LM and NTLM.
...
## How long your account will remain locked before it automatically unlocks
...
## Number of minutes that must elapse before your account locks
...
New-Item -force "HKLM:Software\Policies\Microsoft\Windows NT\CurrentVersion\Winlogon" -Name "MinimumPasswordAge" -Type DWord -Value 14
...
New-Item -force "HKLM:\SOFTWARE\Microsoft\PolicyManager\default\LanmanWorkstation\EnableInsecureGuestLogons" -Name EnableInsecureGuestLogons -Type DWord -Value 0
...
## New-Item -force "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon\GPExtensions\{0ACDD40C-75AC-47ab-BAA0-BF6DE7E7FE63}" -Name "NoGPOListChanges" -Type DWord -Value 0
...
New-Item -force "HKLM:\SOFTWARE\Policies\Microsoft\Windows\WinRM\Client" -Name "AllowUnencryptedTraffic" -Type DWord -Value 0
...
New-Item -force "HKLM:\SOFTWARE\Policies\Microsoft\Windows\WinRM\Service" -Name "AllowUnencryptedTraffic" -Type DWord -Value 0
...
# Windows Server 2022 Internet Protocol version 4 (IPv4) source routing must be configured to the highest protection level to prevent IP source routing.
